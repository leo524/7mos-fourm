<div class="composer">
	...
	<div class="composer-container">
        ...
        <div class="tags-container">
            <input class="tags" size="16" type="text" class="form-control" placeholder="输入美味标签  " tabIndex="2"/>
        </div>

        <!--insert map -->

        <div class="form-row">
            <input type="hidden" class="input-txt inset-shadow" size="50"   id="confirmPlace" >
            <input type="hidden" class="input-txt inset-shadow" id="lat"  placeholder="经度">
            <input type="hidden" class="input-txt inset-shadow" id="lng"  placeholder="纬度">
        </div>
        <!--insert map end-->
        <!-- map title -->
        <div class="form-group">
            <input  readonly id="flocation"  name="maptitle" class="locationtitle  form-control-location"  placeholder="确认美食坐标，点击右侧性感小标标哦"/>
            <!-- <a href="#"   class="searchMap" name="showmap"><img  class="locationpng" src="/images/location.png"></a>-->
            <i class="fa  fa-location-arrow   pull-right searchMap" title="点击查找美食坐标"></i>
        </div>
        <!-- map title end -->
		<!-- ENDIF showTags -->
...
<script type="text/javascript">
    $(document).ready(function() {
        var $ex = $(this),
                $runFirst = $ex.find('.searchMap'),
                codeFirst = $ex.find('.codeFirst').text();
        $runFirst.click(function(e2){
            e2.preventDefault();
            (new Function(codeFirst))();
